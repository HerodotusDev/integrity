...
use integrity::{
    air::{
        layouts::dynamic::{
            ...
            constants::{
                DynamicParams, DynamicParamsIndex as D, segments, PUBLIC_MEMORY_FRACTION, ECDSA_BUILTIN_REPETITIONS,
                PEDERSEN_BUILTIN_REPETITIONS, DILUTED_N_BITS, DILUTED_SPACING, dynamic_params_from_felts
            }
        },
...
impl DynamicAIRCompositionImpl of AIRComposition<InteractionElements, PublicInput> {
    fn eval_composition_polynomial(
        ...
        let public_memory_column_size = trace_domain_size
            / (PUBLIC_MEMORY_FRACTION * (*dynamic_params.at(D::memory_units_row_ratio)).into());
        ...
        // Periodic columns.
        let (pedersen_points_x, pedersen_points_y) = if *dynamic_params.at(D::uses_pedersen_builtin) == 0 {
            (0, 0)
        } else {
            let n_pedersen_hash_copies = trace_domain_size
                / (PEDERSEN_BUILTIN_REPETITIONS * (*dynamic_params.at(D::pedersen_builtin_row_ratio)).into());
        ...
        let (ecdsa_generator_points_x, ecdsa_generator_points_y) = if *dynamic_params
            .at(D::uses_ecdsa_builtin) == 0 {
            (0, 0)
        } else {
            let n_ecdsa_signature_copies = trace_domain_size
                / (ECDSA_BUILTIN_REPETITIONS * (*dynamic_params.at(D::ecdsa_builtin_row_ratio)).into());
        ...
        let (
            ...
            if (*dynamic_params
            .at(D::uses_keccak_builtin)) == 0 {
            (0, 0, 0, 0, 0, 0, 0)
        } else {
            let n_keccak_component_copies = trace_domain_size
                / (DILUTED_N_BITS * (*dynamic_params.at(D::keccak_row_ratio)).into());
        ...
        let (
            ...
            if *dynamic_params
            .at(D::uses_poseidon_builtin) == 0 {
            (0, 0, 0, 0, 0)
        } else {
            let n_poseidon_copies = trace_domain_size / (*dynamic_params.at(D::poseidon_row_ratio)).into();
