...
use integrity::{
    ...
    air::{
        ...
        layouts::dynamic::{
            ...
            constants::{
                segments, get_builtins, CPU_COMPONENT_HEIGHT, LAYOUT_CODE, DynamicParams, DynamicParamsIndex as D,
                PUBLIC_MEMORY_FRACTION, dynamic_params_from_felts
            }
        },
        public_input::{
...
impl DynamicPublicInputImpl of PublicInputTrait {
    fn verify_strict(self: @PublicInput) -> (felt252, felt252) {
        ...
        let layout_builtins = get_builtins(); // TODO: is it ok to assume they are the same?
        let program_builtins = get_builtins();
        ...
        memory
            .verify_stack(
                ...
                program_builtins.len(), // TODO: is it ok to skip layout_builtins
                ref memory_index
            );
        memory
            .verify_stack(
                ...
                stop_addresses.span(),
                program_builtins.len(), // TODO: same
                ref memory_index
            );
    ...
    fn validate(self: @PublicInput, stark_domains: @StarkDomains) {
        let dynamic_params = dynamic_params_from_felts(self.dynamic_params.span());
        ...
        assert(
            n_steps
                ...
                * (*dynamic_params.at(D::cpu_component_step)).into() == trace_length,
            'Wrong trace size'
        );
        ...
        let pedersen_copies = if (*dynamic_params.at(D::uses_pedersen_builtin) == 0) {
            0
        } else {
        ...
        // dynamic_params.at(D::pedersen_builtin_row_ratio)
        // and that stop_ptr - begin_addr is divisible by 3.

        let range_check_copies = if ((*dynamic_params.at(D::uses_range_check_builtin)) == 0) {
            0
        } else {
            trace_length / (*dynamic_params.at(D::range_check_builtin_row_ratio)).into()
        };
        ...
        let ecdsa_copies = if (*dynamic_params.at(D::uses_ecdsa_builtin) == 0) {
            0
        } else {
            trace_length / (*dynamic_params.at(D::ecdsa_builtin_row_ratio)).into()
        };
        ...
        // Note that the following call implies that trace_length is divisible by
        // dynamic_params.at(D::ecdsa_builtin_row_ratio)
        // and that stop_ptr - begin_addr is divisible by 2.

        let bitwise_copies = if (*dynamic_params.at(D::uses_bitwise_builtin) == 0) {
            0
        } else {
            trace_length / (*dynamic_params.at(D::bitwise_row_ratio)).into()
        };
        ...
        // Note that the following call implies that trace_length is divisible by
        // dynamic_params.at(D::bitwise_row_ratio)
        // and that stop_ptr - begin_addr is divisible by 5.

        let ec_op_copies = if (*dynamic_params.at(D::uses_ec_op_builtin) == 0) {
            0
        } else {
            trace_length / (*dynamic_params.at(D::ec_op_builtin_row_ratio)).into()
        };
        ...
        // Note that the following call implies that trace_length is divisible by
        // dynamic_params.at(D::ec_op_builtin_row_ratio)
        // and that stop_ptr - begin_addr is divisible by 7.

        let keccak_copies = if (*dynamic_params.at(D::uses_keccak_builtin) == 0) {
            0
        } else {
            trace_length / (*dynamic_params.at(D::keccak_row_ratio)).into()
        };
        ...
        // Note that the following call implies that trace_length is divisible by
        // dynamic_params.at(D::keccak_row_ratio)
        // and that stop_ptr - begin_addr is divisible by 16.

        let poseidon_copies = if (*dynamic_params.at(D::uses_poseidon_builtin) == 0) {
            0
        } else {
            trace_length / (*dynamic_params.at(D::poseidon_row_ratio)).into()
        };
        ...
        // Note that the following call implies that trace_length is divisible by
        // dynamic_params.at(D::poseidon_row_ratio)
        // and that stop_ptr - begin_addr is divisible by 6.

        let range_check96_copies = if (*dynamic_params.at(D::uses_range_check96_builtin) == 0) {
            0
        } else {
            trace_length / (*dynamic_params.at(D::range_check96_builtin_row_ratio)).into()
        };
        ...
        // Note that the following call implies that trace_length is divisible by
        // dynamic_params.at(D::range_check96_builtin_row_ratio).

        let add_mod_copies = if (*dynamic_params.at(D::uses_add_mod_builtin) == 0) {
            0
        } else {
            trace_length / (*dynamic_params.at(D::add_mod_row_ratio)).into()
        };
        ...
        // Note that the following call implies that trace_length is divisible by
        // dynamic_params.at(D::add_mod_row_ratio)
        // and that stop_ptr - begin_addr is divisible by 7.

        let mul_mod_copies = if (*dynamic_params.at(D::uses_mul_mod_builtin) == 0) {
            0
        } else {
            trace_length / (*dynamic_params.at(D::mul_mod_row_ratio)).into()
        };
        ...
        // Note that the following call implies that trace_length is divisible by
        // dynamic_params.at(D::mul_mod_row_ratio)
        // and that stop_ptr - begin_addr is divisible by 7.

        let memory_units = trace_length / (*dynamic_params.at(D::memory_units_row_ratio)).into();
        ...
        let n_rc_units = trace_length / (*dynamic_params.at(D::range_check_units_row_ratio)).into();
        assert_range_u128_le(
            3 * n_steps + 8 * range_check_copies + 6 * range_check96_copies + 66 * mul_mod_copies,
            n_rc_units,
        );

        let n_diluted_units = trace_length / (*dynamic_params.at(D::diluted_units_row_ratio)).into();
        assert_range_u128_le(68 * bitwise_copies + 16384 * keccak_copies, n_diluted_units);

        if (*dynamic_params.at(D::uses_keccak_builtin) != 0) {
